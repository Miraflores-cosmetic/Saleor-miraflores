...
export function isMenuActive(location: string, menuItem: SidebarMenuItem) {
  ...
  // Если активен URL опубликованных отзывов, не подсвечивать пункт "Отзывы на модерацию"
  if (
    activeUrl === productReviewPublishedUrl().split("?")[0] &&
    menuUrlsToCheck.some(url => url === productReviewListUrl().split("?")[0])
  ) {
    return false;
  }

  // Если активен URL отзывов на модерацию, не подсвечивать пункт "Опубликованные отзывы"
  if (
    activeUrl === productReviewListUrl().split("?")[0] &&
    menuUrlsToCheck.some(url => url === productReviewPublishedUrl().split("?")[0])
  ) {
    return false;
  }
