...
# Проверяем что в конфиге нет общего блокирующего правила для всех скрытых файлов
# Правильно: location ~ /\.git {  (конкретный файл)
# Неправильно: location ~ /\. {  (все скрытые файлы)
if grep -q "location ~ /\\\\\\\.[[:space:]]*{" nginx/nginx.conf || grep -q "location ~ /\\\\\\\.\\$" nginx/nginx.conf; then
    echo "❌ ОШИБКА: В конфигурации найдено общее блокирующее правило 'location ~ /\.'"
    echo "Это правило блокирует ВСЕ файлы начинающиеся с точки, включая .well-known"
    echo "Проверьте что nginx-http.conf обновлен через git pull"
    exit 1
fi

echo "✓ Блокирующих правил не найдено, .well-known разрешен"
